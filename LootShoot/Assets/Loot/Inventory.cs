...
{
    public static List<Loot> loots { get; private set; } = new List<Loot>(); //list for all loot, use list to easily add and remove stuff, would be smarter to use arrays cuz now you can acces loots.Add from other scripts
    public static Loot swapLoot = AllLoot.Empty(); //a slot to put loot in when swaping placements in inventory
    //should make a loot for and instead of using loots[0]
